...
module faceplate(
    ...
    module ribs(plate_size, parts_size) {
        tag("rib") align(TOP, [FRONT,BACK]) 
            prismoid(size2=[parts_size.x, rib_width], h=rib_depth, xang=30, yang=90);
    }

    // Layout calculations
    height = rack_units * rack_1u_height;
    plate_size = [rack_width_10inch, height, faceplate_thickness];
    parts_area_width = plate_size.x - 2*shelf_tab_width_tecmojo - 2*rib_width;
    parts_area_height = plate_size.y - 2*rib_width;
    ...
    diff(remove="cutout") {
        extruded_roundrect(plate_size, r = faceplate_rounding, anchor=TOP) {
            ...
            if (add_ribs) {
                ribs(plate_size, parts_area_size);
            }
            ...
            // Parts
            if (!is_undef(part)) {
                options = render_options(add_ribs);
                render_parts(part = part, part_size = parts_area_size, options = options);
            }
        }
    }
}